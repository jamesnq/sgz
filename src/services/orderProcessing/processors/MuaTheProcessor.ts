...
export const MuaTheMetadataSchema = BaseMetadataSchema.extend({
  type: z.literal('muathe'),
  region: z.string().min(1),
  denomination: z.number().positive(),
})

export type MuaTheMetadata = z.infer<typeof MuaTheMetadataSchema>

export class MuaTheProcessor implements OrderProcessor {
  ...
  private validateMetadata(order: Order): {
    ...
    try {
      ...
      const validatedData = MuaTheMetadataSchema.parse(productVariant.metadata)
  ...
  private async purchaseSteamWalletCode(_metadata: MuaTheMetadata): Promise<string> {
